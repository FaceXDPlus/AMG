...
namespace MaterialUI
{
    ...
    public class SnackbarManager : MonoBehaviour
    {
		...
		/// <summary>
		/// The queue max size (if more snackbars are to be show after this limit, they will be ignored)
		/// </summary>
		[SerializeField]
        ...
        /// </summary>
		private Queue<Snackbar> m_SnackbarQueue;
        /// <summary>
        ...
        private void InitSystem()
        {
            ...
            m_SnackbarQueue = new Queue<Snackbar>();
        ...
        public static void Show(string content, float duration, Color panelColor, Color textColor, int fontSize, string actionName, Action onActionButtonClicked)
        {
			...
			int toastTotalCount = instance.m_SnackbarQueue.Count + (m_Instance.m_IsActive ? 1 : 0);
			if (toastTotalCount >= instance.m_MaxQueueSize)
			{
				return;
			}

            instance.m_SnackbarQueue.Enqueue(new Snackbar(content, duration, panelColor, textColor, fontSize, actionName, onActionButtonClicked));
            instance.StartQueue();
        }
        ...
        {
            if (m_SnackbarQueue.Count > 0 && !m_IsActive)
            {
                m_CurrentAnimator.Show(m_SnackbarQueue.Dequeue());
                m_IsActive = true;
            }
        }
        ...
        public static bool OnSnackbarCompleted()
        {
            ...
            instance.StartQueue();
            return (instance.m_SnackbarQueue.Count > -1);
        }
    }
}