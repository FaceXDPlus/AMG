...
namespace MaterialUI
{
    ...
    public class Ripple : Image
    {
        ...
        [SerializeField] private Material m_ShadowedRippleMaterial;

        private static Material m_SharedRippleMaterial;
        private static Material m_SharedShadowedRippleMaterial;
        ...
        public static void ResetMaterial()
        {
            if (!m_MaterialIsReset)
            {
                ...
                if (m_SharedRippleMaterial == null)
                {
                    m_SharedRippleMaterial.SetFloat("_Softening", 0.25f);
                }

                if (m_SharedShadowedRippleMaterial == null)
                {
                    m_SharedShadowedRippleMaterial.SetFloat("_Softening", 0.25f);
                }
            }
        }
        ...
        public void Create(int id, VectorImageData imageData)
        {
            ...
            if (m_SharedRippleMaterial == null)
            {
                m_SharedRippleMaterial = m_RippleMaterial;
            }

            if (m_SharedShadowedRippleMaterial == null)
            {
                m_SharedShadowedRippleMaterial = m_ShadowedRippleMaterial;
            }
        }
        ...
        private static void CalculateAverageSoftening()
        {
            ...
            m_SharedRippleMaterial.SetFloat("_Softening", 0.5f * (float.IsNaN(autoSoftenValue) ? 0.25f : autoSoftenValue));
            m_SharedShadowedRippleMaterial.SetFloat("_Softening", 0.5f * (float.IsNaN(autoSoftenValue) ? 0.25f : autoSoftenValue));
    ...
    public class RippleData
    {
        ...
        /// <summary>
        /// Should the ripple have a shadow?
        /// </summary>
        public bool Shadow;
