...
namespace MaterialUI
{
    ...
    /// <summary>
    /// Text validator that checks for exact length.
    /// </summary>
    /// <seealso cref="MaterialUI.BaseTextValidator" />
    /// <seealso cref="MaterialUI.ITextValidator" />
    public class ExactLengthTextValidator : BaseTextValidator, ITextValidator
    {
        /// <summary>
        /// The error message.
        /// </summary>
        ...
        /// The minimum length
        /// </summary>
        private int m_ExactLength = 6;

        /// <summary>
        /// Initializes a new instance of the <see cref="ExactLengthTextValidator"/> class.
        /// </summary>
        /// <param name="exactLength">The exact length.</param>
        /// <param name="errorMessage">The error message.</param>
        public ExactLengthTextValidator(int exactLength, string errorMessage)
        {
            m_ExactLength = exactLength;

            if (string.IsNullOrEmpty(errorMessage))
            {
                m_ErrorMessage = "Requires exactly " + m_ExactLength + " characters";
            }
            else
            {
                m_ErrorMessage = errorMessage;
        ...
        /// Determines whether the text is valid.
        /// </summary>
        /// <returns>True if the text length is exactly m_ExactLength, otherwise false.</returns>
        public bool IsTextValid()
        {
            if (m_MaterialInputField.inputField.text.Length != m_ExactLength)
            {
                m_MaterialInputField.validationText.text = m_ErrorMessage;
                return false;
            }
